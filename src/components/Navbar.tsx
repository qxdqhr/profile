...
const NavbarItem: React.FC<{ item: NavItem, direction: 'horizontal' | 'vertical' }> = ({ item, direction }) => {
...
const Navbar: React.FC<NavbarProps> = ({ direction = 'horizontal', items, isOpen }) => {
    ...
    return (
        <nav ref={navbarRef} className={`navbar ${direction} ${isOpen ? 'open' : 'closed'}`}>
            <Avatar
